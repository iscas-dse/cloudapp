 ...
 * @author RichardLcc
 * @date 2015年1月29日-上午11:00:23
 */
@Service
public class EdgeTypedGraphOperationBuilder {
	private Logger logger=LoggerFactory.getLogger(EdgeTypedGraphOperationBuilder.class);
	...
	public Operation createNewOperation(Graph<Node, EdgeType> graph,
		...
		logger.info("进入createNewOperation--operationName:"+operationName);
		...
		logger.debug(defineFileContent);
	 ...
	 */
	@SuppressWarnings({ "rawtypes", "unchecked" })
	public String createDefineContent(Graph<? extends Node, EdgeType> graph,Set<Operation> imports, Map<String,ParamValue> extractParams){
		Preconditions.checkArgument(graph != null,
				"创建Operation出错：传入的参数graph为null");
		Preconditions.checkArgument(graph.size() > 0,
				"创建Operation出错：传入的参数graph的size为空");
		...
		if (list!=null&&list.size() > 0) {
			...
			for (Edge<? extends Node,EdgeType> edge : list) {
				...
				if (node instanceof ModuleClass) {
					...
					builder.append(defineName + "[\"" + node.getNodeName()
							+ "_${name}\"],");
			...
			if(notifyBuilderCount>0){
				if(notifyBuilderCount>1){
					notifyBuilder.insert(0, "notify => [");
					notifyBuilder.append("],");
				}
				else
					notifyBuilder.insert(0,"notify =>");
				notifyBuilder.append(newline);
			}
			int count=(list.size()-notifyBuilderCount-nullCount);
			if(count>0){
				if(count>1){
					requireBuilder.insert(0, "require => [");
